...
kube-apiserverがClushBackOffを繰り返しているので、その原因を探ったうえで正常に起動させなさい。
動かなくなった原因は、今まで使用していたものからのバージョンアップを行ったのが原因になっているが、それまでは
外部etcdを使用したものであった。

まずは、kube-apiserverを起動させて、kubectlコマンドを使用できるようにしなさい。
その上で、対象のNodeにスケジューリングされているPodすべてがReadyになるように再設定を実施しなさい。
